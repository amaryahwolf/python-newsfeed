...
# Use Base class with the engine connection variable to drop all existing tables, then create any tables that Base mapped
Base.metadata.drop_all(engine)
Base.metadata.create_all(engine)

# Establish a temporary session connection with the Session class
db = Session()

# insert users
# Use add_all() method and the User model to create several new users
db.add_all([
...
# Call to db.commit() to insert statements
db.commit()
...
# Close the session connection after db transactions complete
db.close()